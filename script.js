// --- Fast-Safe City Navigator (script.js) ---
...
// --- Risk map (smooth blobs + day/night tweak)
...
// --- Build adjacency (captures blocks + risk + directions)
function buildAdj() {
  const adj = new Map();
...
class MinPQ {
  ...
  pop(){
    ...
    if (this.a.length){ this.a[0] = e; this._down(0); }
    return t;
  }
  _up(i){
    for (; i; ){
  ...
  _down(i){
    for(;;){
      ...
      if (l < this.a.length && this.a[l].key < this.a[s].key) s = l;
      if (r < this.a.length && this.a[r].key < this.a[s].key) s = r;
      if (s === i) break;
...
function draw(){
  ...
  for (let c = 0; c <= cols; c++){
    ...
    ctx.beginPath(); ctx.moveTo(x, padY); ctx.lineTo(x, padY + rows * cellH); ctx.stroke();
  }
  ...
  if (lastPath && lastPath.length){
    ...
    ctx.beginPath();
    const [sr, sc] = rcOf(lastPath[0]);
    const [sx, sy] = cellRect(sr, sc);
    ctx.moveTo(sx + (cellW / 2), sy + (cellH / 2));
    for (let i = 1; i < lastPath.length; i++){
      const [r, c] = rcOf(lastPath[i]);
